        ...
        for( j=0; j<8; j++ )
        {
            ...
            if( ((tmp_char >> (7 - j)) & 0x01) == 0x01 )
            {
                i=i*size;
							...
							if(size==2||size==3){
							  LCD_SetPoint( Xpos + j+1, Ypos + i, charColor );  /* 字符颜色 */
							  LCD_SetPoint( Xpos + j, Ypos + i+1, charColor );  /* 字符颜色 */
							  LCD_SetPoint( Xpos + j+1, Ypos + i+1, charColor );  /* 字符颜色 */
            ...
            else
            {
                i=i*size;
								j=j*size;
								LCD_SetPoint( Xpos + j, Ypos + i, bkColor );  /* 字符颜色 */
								if(size==2||size==3){
									LCD_SetPoint( Xpos + j+1, Ypos + i, bkColor );  /* 字符颜色 */
									LCD_SetPoint( Xpos + j, Ypos + i+1, bkColor );  /* 字符颜色 */
									LCD_SetPoint( Xpos + j+1, Ypos + i+1, bkColor );  /* 字符颜色 */
								}
							  if(size==3){
									LCD_SetPoint( Xpos + j-1, Ypos + i, bkColor );  /* 字符颜色 */
									LCD_SetPoint( Xpos + j, Ypos + i-1, bkColor );  /* 字符颜色 */
...
* Attention		 : None
*******************************************************************************/
void GUI_Text(uint16_t Xpos, uint16_t Ypos, const char *str,uint16_t Color, uint16_t bkColor, int size)
{
    uint8_t TempChar;
    do
    {
