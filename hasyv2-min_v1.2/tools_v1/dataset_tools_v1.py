...
###1. fun make_hasy_dataset(dataset_path, csv_path)
...
def make_hasy_dataset(dataset_path, csv_path):
	images_hasy = []
	labels_hasy = []
...
def make_x_y_split(source_img_dataset_array, train_perc):
	...
	effective_train_len = math.trunc(train_len)
	effective_train_len += 1 #avoid to lose one sample after the division, the choose is to keep it in the training split

	effective_test_len = math.trunc(test_len)

	print(f"The dataset len is: {entire_dataset_len}\n TRAIN SPLIT: {effective_train_len} samples\nTEST SPLIT: {effective_test_len}\n\n")
